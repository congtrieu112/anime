...
Phim3s.Search.AutoComplete = function($element) {
	...
	this.url = 'http://localhost/ami/ajax/'
...
Phim3s.Watch = {
	...
	precheckIsFanOfPage: function(userId) {
		var query = FB.Data.query("SELECT uid FROM page_fan WHERE page_id = 1481358568748904 and uid="+userId);
	...
	registerFacebookEvent: function() {
		...
		window.fbAsyncInit = function() {
			FB.init({
			  appId      : '1481358568748904',
			  channelUrl : "http://localhost/ami/", 
			  status     : true, 
			  cookie     : true,
			  xfbml      : true  
			});
