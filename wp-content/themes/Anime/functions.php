...
/*  Hien thi "x ... truoc" cho Post va Comment
/* ------------------------------------------------------------------------- */
function time_ago( $type = 'post' ) {
    $d = 'comment' == $type ? 'get_comment_time' : 'get_post_time';
    return human_time_diff( $d('U'), current_time('timestamp') ) ." ". __( 'ago', 'hueman' );
}
...
function get_views_last($type,$num=20){
			global $wpdb;
...
<div class="views">Lượt xem '.($views*3).'</div>
</div>
</li>
';
}
...
function get_most_viewed_2($limit = 16,$begin=0,$excerpt_length=40, $show_thumb = true) {
		global $wpdb, $post;
	$type_alohot	=	$type;
	$file = DOCUMENT_TEMP."/cached/alohot_mostview.xxx"; // lấy tên file cache theo type để tránh trùng lập
	$expire = 86400; // 24h
	if (file_exists($file) &&
    filemtime($file) > (time() - $expire)) {
    include(DOCUMENT_TEMP."/cached/alohot_mostview.xxx");
	} else { 		
		$where = "";
		$output = '';
		...
		$html.='';
			 foreach ($most_viewed as $post) { 
				...
				$post_title = get_the_title();
					$html .= '<li><div class="inner"><a href="'.get_permalink().'" title="'.get_the_title().'"><img src="'.img3(146,195).'" alt="'.get_the_title().'"/></a><div class="info"><div class="name"><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a> </div><div class="name2">'.get_post_meta($post->ID, "phim_en", true).'</div><div class="stats"></div></div>
					
					<!--
					<div class="status">'.get_post_meta($post->ID, "phim_hd", true).'</div>
					<div class="g-status">'.get_post_meta($post->ID, "phim_nsx", true).'</div></div></li>
					-->
					<div class="f_tag">
	                    <div class="f_t_f"></div>
	                    <div class="f_t_c">'.get_post_meta($post->ID, "phim_hd", true).'</div>
	                    <div class="f_t_e"></div>
	                    <div class="clr"></div>
	                </div>
...
function get_excerpt_content($max_char = 55,$more_text = '', $printout = 1,$content = '') {
	...
	$words = explode(' ', $content, $max_char + 3);
...
global $post;
	...
	$file = DOCUMENT_TEMP."/cached/alohot_".$type.".xxx"; // lấy tên file cache theo type để tránh trùng lập
	$expire = 86400; // 24h
	if (file_exists($file) &&
    filemtime($file) > (time() - $expire)) {
