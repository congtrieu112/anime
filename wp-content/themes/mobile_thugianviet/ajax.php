...
$idphim=$_GET['film_id'];
$tap=get_name($idtap);
$name=get_the_title($idphim);
if($idtap!=0){
	$member = array(
					'html' => getlinkphim($idphim,$idtap,''),
					'server_name'=>'vip',
					'episode_name'=>$tap,
					'title'=>"Xem phim ".$name." tập ".$tap
                   );
   echo json_encode($member);  
   die;
}
...
if($q){
foreach ($q as $box) {
$html.='<li><strike data-film-id="'.$box->box_phim.'" onclick="Phim3s.Member.removeBookmark('.$box->box_phim.'); return false;" ></strike><a href="'.get_permalink($box->box_phim).'" title="'.get_the_title($box->box_phim).'">'.get_the_title($box->box_phim).'</a></li>';
}
}
	$member = array(
...
echo 1;
}elseif($_POST['broken']==1){
global $post;
if(!$_SESSION['broken'])
	$insert = $wpdb->query ( "UPDATE " . DATA_FILM_EPISODE . " 
										SET  
											episode_broken = 1
											
										WHERE episode_id = '" . $_POST['epid'] . "'" );
		if ($insert) {
		$_SESSION['broken']=$insert->episode_broken;
