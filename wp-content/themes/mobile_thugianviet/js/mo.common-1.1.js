...
var cookie_like = false;
var cookie_dislike = false;
$('.fn-err').on('click',function(e){
	var o = $(this);
	$.getJSON("/ajax.php?mode=err&idmov="+o.attr('id-mov'), function(data){
		$('#fnMsgError').html("Báo lỗi thành công");
	});
	$('#fnShareBox').addClass('none');
	$('#fnWrapper').addClass('none');
	$('#fnMsgBox').removeClass('none');
	$('#fnPopup').removeClass('none');
	return false;
});
$('.fn-like').on('click',function(e){
	var o = $(this);
	if(cookie_like == false) {
		$.getJSON("/ajax.php?mode=fav&idmov="+o.attr('id-mov'), function(data){
			if(data.echo == 1) {
				o.addClass('liked');
				cookie_like = true;
			}
...
$('.liked').on('click',function(e){
	var o = $(this);
	if(cookie_dislike == false) {
		$.getJSON("/ajax.php?mode=rfav&idmov="+o.attr('id-mov'), function(data){
			if(data.echo == 1) {
				$('#fnLike').removeClass('liked');
				cookie_dislike = true;
			}
		});
	}
	//$('#fnMsgError').html("Chức năng tạm thời bảo trì");
	//$('#fnShareBox').addClass('none');
	//$('#fnWrapper').addClass('none');
	//$('#fnMsgBox').removeClass('none');
	//$('#fnPopup').removeClass('none');
...
var MoSearch = {
			...
			if(k){
				...
				for(var i=0; i<kHistory.length; i++){
					if(kHistory[i].trim().toLowerCase().search(k) !=-1) {
						items+='<li><a href="/search/'+encodeURI(kHistory[i])+'/">'+kHistory[i]+'</a></li>';
					}
				}
			} else {
				for(var i=0; i<kHistory.length; i++){
	...
	getSuggest:function(k){
		$('#fnSuggest').removeClass('none');
		$.getJSON("/search.php?key="+k, function(data){
				$('#fnSuggest').html('<ul>'+data.timkiem+'</ul>');
		});
	},
