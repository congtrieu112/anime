...
function nhanweb_pstlinkgf_get_thumbnail($width=100, $height=100, $custom_field='', $post='')
{
	...
	if ($thumb_array['thumb'] == '') {
		if ($custom_field == '') $thumb_array['thumb'] = get_post_meta($post->ID, 'Image', $single = true);
		else { 
			$thumb_array['thumb'] = get_post_meta($post->ID, $custom_field, $single = true);
			if ($thumb_array['thumb'] == '') $thumb_array['thumb'] = get_post_meta($post->ID, 'Image', $single = true);
		}
			...
			elseif ( $custom_field <> '' && get_post_meta($post->ID, 'Thumbnail', $single = true) ) 
				$thumb_array['thumb'] = apply_filters( 'et_fullpath', get_post_meta($post->ID, 'Image', $single = true) );
		}	
	return $thumb_array;
}
/**********************************************************
